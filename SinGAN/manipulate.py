...
def generate_gif(Gs,Zs,reals,NoiseAmp,opt,alpha=0.1,beta=0.9,start_scale=2,fps=10):
    ...
    for G,Z_opt,noise_amp,real in zip(Gs,Zs,NoiseAmp,reals):
        ...
        images_cur = []
        if count == 0:
            z_rand = functions.generate_noise(Image_tensor,[1,nzx,nzy], device=opt.device)
            z_rand = z_rand.expand(1,3,Z_opt.shape[2],Z_opt.shape[3])
            z_prev1 = 0.95*Z_opt +0.05*z_rand
            z_prev2 = Z_opt
        else:
            z_prev1 = 0.95*Z_opt +0.05*functions.generate_noise(Image_tensor,[opt.nc_z,nzx,nzy], device=opt.device)
            z_prev2 = Z_opt
        ...
        for i in range(0,100,1):
            if count == 0:
                ...
                diff_curr = beta*(z_prev1-z_prev2)+(1-beta)*(functions.generate_noise(Image_tensor,[opt.nc_z,nzx,nzy], device=opt.device))
